...
describe('NBack logic', () => {
  it('generates a sequence of the requested length', () => {
    const length = roundsForN(START_N)
    const { sequence } = generateSequence(length, 1)
  ...
  it('increases N after a winning run', () => {
    const result = nextDifficulty(ADVANCE_ACCURACY, START_N, roundsForN(START_N))
    assert.strictEqual(result.n, START_N + 1)
    assert.strictEqual(result.length, roundsForN(START_N + 1))
  })

  it('round count scales with N', () => {
    assert.strictEqual(roundsForN(1), 5)
    assert.strictEqual(roundsForN(2), 10)
    assert.strictEqual(roundsForN(6), MAX_LENGTH)
  })
